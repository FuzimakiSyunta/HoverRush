...
public class GameManager : MonoBehaviour
{
    ...
    private void FixedUpdate()//敵出現
    {
        ...
        if (GameStartFlag == true&&BossWaveFlag==false) // ゲームが開始しており、現在のWaveが0である場合
        {
            int RandomEnemy = Random.Range(0, 15000); // 敵の出現頻度をランダムに決定
            int Style = Random.Range(0, 3); // 敵の種類（見た目や特徴）をランダムに決定
            ...
            for (int i = 0; i < Enemyranges.Length; i++)
            {
                ...
                if (RandomEnemy >= Enemyranges[i][0] && RandomEnemy <= Enemyranges[i][1])
                {
                    ...
                    if (CoolTime[i] >= 0.5f)
                    {
                        ...
                        {
                            Instantiate(enemy, positions[i], Quaternion.identity); // 通常敵を生成
                        }
                        // 隕石型の敵を生成
                        else if (Style == 1)
                        {

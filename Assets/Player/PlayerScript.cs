...
public class PlayerScript : MonoBehaviour
{
    ...
    {
        if ((other.gameObject.CompareTag("Lazer") || other.gameObject.CompareTag("FinalLazer")) && damageTimer >= damageCooldown)
        {
            // ダメージ処理（0.1秒ごと）
            ...
            hpSlider.value = (float)currentHp / (float)maxHp;
            damageTimer = 0f; // ダメージタイマーリセット
        }

        if ((other.gameObject.CompareTag("Lazer") || other.gameObject.CompareTag("FinalLazer")) && particleTimer >= particleCooldown)
        {
            ParticleSystem newParticle = Instantiate(LazerParticle, transform.position, Quaternion.identity);
            newParticle.Play();
            Destroy(newParticle.gameObject, 5.0f);

            particleTimer = 0f; // パーティクルタイマーリセット
        }
    }

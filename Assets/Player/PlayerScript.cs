...
public class PlayerScript : MonoBehaviour
{
    ...
    void PlayerHeal()
    {
        ...
        if (gameManagerScript.GetHealBatteryEnargy() >= 9 && isHeal == false && currentHp < 500)
        {
            ...
            if (Input.GetKeyDown(KeyCode.L) || Input.GetKeyDown("joystick button 2"))
            {
                ...
                {
                    currentHp += 150; // 通常の増加処理
                    if (currentHp > 300)
                    {
                        currentHp = 300; // 500を超えた場合は500にリセット
                    }
                }
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        if (other.gameObject.tag == "FinalLazer"|| other.gameObject.tag == "Lazer")
        {
            // パーティクルシステムのインスタンスを生成する。
            ParticleSystem newParticle = Instantiate(LazerParticle);
            // パーティクルの発生場所をこのスクリプトをアタッチしているGameObjectの場所にする。
            newParticle.transform.position = this.transform.position;
            // パーティクルを発生させる。
            newParticle.Play();
